...
function seleccionarResultado(
    ...
    // Verificar si ya hay un número de expediente
    var numero_expediente = document.getElementById("numero_expediente").value;
    if (!numero_expediente) {
      // Si no hay número de expediente, generarlo aquí
      // Luego, asignar el número de expediente generado al campo correspondiente
      var nuevo_numero_expediente = generarNumeroExpediente(); // Función para generar el número de expediente
      document.getElementById("numero_expediente").value = nuevo_numero_expediente;
    }
  
    // Limpiar el contenido del elemento de resultados de búsqueda
    document.getElementById("resultado-busqueda").innerHTML = "";
  
    // Deshabilitar campos de apellido paterno y apellido materno
  ...
  function buscarNombre(str) {
    if (str.length == 0) {
      ...
      xmlhttp.open("GET", "buscar_nombre2.php?q=" + str, true);
      xmlhttp.send();
    }
  }
